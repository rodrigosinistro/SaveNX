...
namespace curl
{
    /// @brief SaveNX's user agent string.
    static constexpr const char *STRING_USER_AGENT = "SaveNX";
    ...
    static inline void reset_handle(curl::Handle &curl)
    {
        ...
        curl::set_option(curl, CURLOPT_CAINFO, "romfs:/certs/cacert.pem");
        curl::set_option(curl, CURLOPT_USERAGENT, curl::STRING_USER_AGENT);
        curl::set_option(curl, CURLOPT_CONNECTTIMEOUT, 15L);
        curl::set_option(curl, CURLOPT_FOLLOWLOCATION, 1L);
        curl::set_option(curl, CURLOPT_MAXREDIRS, 5L);
        curl::set_option(curl, CURLOPT_PROTOCOLS, static_cast<long>(CURLPROTO_HTTPS));
        curl::set_option(curl, CURLOPT_REDIR_PROTOCOLS, static_cast<long>(CURLPROTO_HTTPS));
    }
    ...
    /// @brief Curl callback that writes a download directly to its destination and updates progress.
    /// @param buffer Incoming buffer from CURL.
    /// @param size Element size.
    /// @param count Element count.
    /// @param download Download state containing the destination and optional task.
    /// @return Number of bytes written, or zero to abort the transfer on a write error.
    size_t write_download_to_file(const char *buffer, size_t size, size_t count, curl::DownloadStruct *download);
