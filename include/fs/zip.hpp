...
namespace fs
{
    ...
    bool copy_directory_to_zip(const fslib::Path &source, fs::MiniZip &dest, sys::ProgressTask *Task = nullptr);

    /// @brief Unzips source to destination.
    /// @note Task is optional.
    bool copy_zip_to_directory(fs::MiniUnzip &source,
    ...
    /// @brief Preflights every entry, rejects path traversal and verifies each ZIP CRC before restore.
    /// @note Resets the archive to its first entry before returning.
    bool validate_zip_archive(fs::MiniUnzip &unzip);
} // namespace fs
