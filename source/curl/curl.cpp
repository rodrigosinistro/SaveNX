...
size_t curl::write_download_to_file(const char *buffer, size_t size, size_t count, curl::DownloadStruct *download)
{
    if (!download || !download->dest) { return 0; }

    const size_t downloadSize = size * count;
    const ssize_t written     = download->dest->write(buffer, downloadSize);
    if (written < 0 || static_cast<size_t>(written) != downloadSize) { return 0; }

    if (download->task) { download->task->update_current(static_cast<double>(download->dest->tell())); }
    return downloadSize;
}
