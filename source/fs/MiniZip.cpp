...
bool fs::MiniZip::close()
{
    if (!m_isOpen) { return true; }
    const bool closed = zipClose(m_zip, nullptr) == ZIP_OK;
    m_isOpen = false;
    return closed;
}

bool fs::MiniZip::add_directory(std::string_view filename)
{
    if (filename.empty()) { return false; }
    std::string zipPath{filename};
    if (zipPath.back() != '/') { zipPath.append("/"); }

    return MiniZip::open_new_file(zipPath) && MiniZip::close_current_file();
}
