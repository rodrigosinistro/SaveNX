...
void tasks::saveimport::import_save_backup(sys::threadpool::JobData taskData)
{
    ...
    const bool typeMatches = metaData.saveDataType == user->get_account_save_type();
    const AccountUid selectedUid = user->get_account_id();
    const bool uidMatches = metaData.saveDataType != FsSaveDataType_Account ||
                            (metaData.accountID.uid[0] == selectedUid.uid[0] &&
                             metaData.accountID.uid[1] == selectedUid.uid[1]);
    if (!typeMatches || !uidMatches) { TASK_FINISH_RETURN(task); }
    ...
    if (error::is_null(saveInfo) || !fs::validate_save_meta_data(saveInfo, metaData)) { TASK_FINISH_RETURN(task); }

    {
        ...
        else
        {
            ...
            const bool restored = fs::copy_zip_to_directory(unzip, fs::DEFAULT_SAVE_ROOT, metaData.journalSize, task);
            if (!restored)
            {
                const char *popWriteError = strings::get_by_name(strings::names::IO_POPS, 0);
                ui::PopMessageManager::push_message(ui::PopMessageManager::DEFAULT_TICKS, popWriteError);
...
static bool read_save_meta(const fslib::Path &path, fs::SaveMetaData &metaOut)
{
    ...
    else
    {
        ...
        if (!zip.is_open() || !fs::validate_zip_archive(zip)) { return false; }
